...
TC_connect.prototype.tcLogin = function(callback, authenticatedMethod)
{
    ...
    //if the session is initialized and within token timeout, no need to re-auth
    if(that.sessionDateTime != null && ((that.sessionDateTime + (1000 * that.sessionTimeout)) > Date.now())) {
    ...
    /*
    I'm not going to worry about refreshing the token right now and see how it goes - notes:    
    - need to refresh token within timeout (2 minutes default) - 
        I would be refreshing every two minutes async
        wondering if there would be collisions (is it multi threaded?)
        wondering if my session would stay valid that long (but it might renew it itself eventually...)
        does the whole token expire after some time and I need a new login anyway?
    - if I don't... after timeout I do a new login
        the downside is a new session is created every time 
        I can't figure out how to call PersistSession from my code... some commented out attempts below
            I think that would hold on to the session 
        I'm not calling KillSession
    */
    //
    //
    // var refreshing = this.sessionTimeout != -1;//if it's -1, we are getting a brand new token - otherwise, it's a refresh. 

    //testing, refresh if it is < 2 minutes
    // var refreshing = that.sessionDateTime != null && ((that.sessionDateTime + (1000 * that.sessionTimeout)) > Date.now()); 
    ...
    request.post({
        ...
        if (!err && typeof response !== 'undefined' && response.statusCode == 200) {
            ...
            if(result["access_token"] == null) {
                ...
                //         headers: { __requestverificationtoken: `Bearer ${that.accessToken}`},
